...
export default function MovieDetailPageClient({ movie }: { movie: ReleasedMovie }) {
  ...
  return (
    <div className="min-h-screen bg-[#0a0a14] text-white">
      ...
      {/* ── Hero / Backdrop ── */}
      <section className="relative w-full h-[68svh] min-h-[560px] overflow-hidden md:h-[68vh] md:min-h-0">
        ...
        <div className="absolute bottom-0 left-0 right-0 p-4 sm:p-6 md:p-10 flex items-end gap-6 md:gap-8 z-10">
          {/* Poster thumbnail */}
          {movie.poster && (
            <motion.div
          ...
          >
            ...
            <h1 className="text-3xl sm:text-4xl md:text-6xl font-black text-white leading-tight mb-1 drop-shadow-2xl break-anywhere">
              {movie.title}
            </h1>
            {movie.releaseDate && (
            ...
            <div className="flex flex-col gap-3 min-[420px]:flex-row min-[420px]:flex-wrap">
              {movie.trailer && (
                <a
                  ...
                  className="inline-flex items-center justify-center gap-2 px-5 py-2.5 rounded-xl bg-red-600 hover:bg-red-700 text-white font-semibold text-sm transition-all shadow-lg hover:shadow-red-500/30 hover:scale-[1.02] sm:px-6"
                >
                  ▶ Watch Trailer
                </a>
              )}
              {(movie.ticketLinks ?? []).filter(t => t.available).slice(0, 2).map(t => (
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="inline-flex items-center justify-center gap-2 px-5 py-2.5 rounded-xl bg-yellow-500 hover:bg-yellow-400 text-black font-bold text-sm transition-all shadow-lg hover:shadow-yellow-500/30 hover:scale-[1.02] sm:px-6"
                >
                  🎟 {t.platform}
                </a>
              ))}
