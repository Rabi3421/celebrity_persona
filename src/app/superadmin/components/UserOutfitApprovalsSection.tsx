    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-start gap-3">
        <div className="flex-1 min-w-0">
          <h2 className="font-playfair text-2xl sm:text-3xl font-bold text-white mb-1">User Outfit Approvals</h2>
          <p className="text-neutral-400 text-sm">Review and approve fashion items submitted by users</p>
        </div>
        {counts.pending > 0 && (
          <div className="flex items-center gap-2 px-4 py-2 bg-yellow-500/10 border border-yellow-500/30 rounded-2xl self-start">
            <div className="w-2 h-2 rounded-full bg-yellow-400 animate-pulse" />
            <span className="text-yellow-400 text-sm font-medium whitespace-nowrap">{counts.pending} awaiting review</span>
          </div>
        )}
      </div>

      {/* Tabs */}
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center border-b border-white/10 pb-3">
        <div className="flex items-center gap-1 overflow-x-auto flex-wrap">
          {tabs.map((t) => (
            <button
              key={t.id}
              onClick={() => { setTab(t.id); setPage(1); }}
              className={`flex items-center gap-2 px-4 py-2 rounded-t-xl text-sm font-medium transition-all whitespace-nowrap ${
                tab === t.id
        ...
        <div className="flex items-center gap-2 sm:ml-auto">
          <input
            ...
            className="glass-card px-4 py-2 rounded-xl text-sm text-white placeholder:text-neutral-500 focus:outline-none focus:ring-2 focus:ring-yellow-500/40 bg-transparent w-full sm:w-48"
          />
          <button
            onClick={() => fetchOutfits(1, tab, search)}
            className="glass-card p-2 rounded-xl hover:bg-white/10 transition-colors shrink-0"
          >
