...
export default function AggregatedScores({ ratings }: AggregatedScoresProps) {
  return (
    <div className="glass-card p-8 rounded-3xl mb-12 animate-fade-in-up">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        ...
        <div className="glass-card p-6 rounded-2xl hover:bg-white/5 transition-all">
          <div className="flex items-center gap-3 mb-4">
            <div className="w-12 h-12 rounded-xl bg-primary/20 flex items-center justify-center">
              <Icon name="UsersIcon" variant="solid" size={24} className="text-primary" />
            </div>
            <div>
              <h3 className="text-white font-bold text-sm">User Reviews</h3>
              <p className="text-neutral-400 text-xs">{ratings.totalReviews} reviews</p>
            </div>
          </div>
          <div className="flex items-baseline gap-2 mb-3">
            ...
            <span className="text-neutral-400 text-xl">/5</span>
          </div>
          <div className="flex items-center gap-1">
            {Array.from({ length: 5 }).map((_, index) => (
              <Icon
                key={index}
                name="StarIcon"
                variant={index < Math.floor(ratings.averageRating) ? 'solid' : 'outline'}
                size={16}
                className={
                  index < Math.floor(ratings.averageRating)
                    ? 'text-primary' :'text-neutral-600'
                }
              />
            ))}
          </div>
